...
func exportAppStateAndTMValidators(
	...
	if height != -1 {
		simApp = app.NewSimApp(logger, db, traceStore, false, map[int64]bool{}, viper.GetString(cli.HomeFlag), uint(1), getContractHandler(viper.GetString(flagContractMode)), app.DefaultChannelResolverProvider, app.DefaultAnteHandlerProvider, nil)
		err := simApp.LoadHeight(height)
		if err != nil {
			return nil, nil, nil, err
		}
	} else {
		simApp = app.NewSimApp(logger, db, traceStore, true, map[int64]bool{}, viper.GetString(cli.HomeFlag), uint(1), app.DefaultContractHandlerProvider, app.DefaultChannelResolverProvider, app.DefaultAnteHandlerProvider, nil)
	}
